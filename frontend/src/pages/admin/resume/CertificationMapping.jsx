// frontend/src/pages/admin/resume/CertificationMapping.jsx
...
const CertificationMapping = () => {
  ...
  const fetchCertifications = async (searchVal = "") => {
    setLoading(true);
    try {
      const res = await axiosInstance.get(`/admin/resume-intelligence/certifications?search=${searchVal}`);
      setCertifications(res.data.certifications);
    } catch (err) {
      showToast("Failed to fetch certification mappings.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openAddModal = () => {
    setEditingId(null);
    setFormData({ certificationName: "", mappedSkills: [] });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.certificationName.trim() || formData.mappedSkills.length === 0) {
      showToast("Please provide a certification name and select at least one skill.", "error");
      return;
    }

    try {
      if (editingId) {
        await axiosInstance.put(`/admin/resume-intelligence/certifications/${editingId}`, formData);
        showToast("Certification mapping updated successfully!", "success");
      } else {
        await axiosInstance.post("/admin/resume-intelligence/certifications", formData);
        showToast("Certification mapping created successfully!", "success");
      }
      setShowFormModal(false);
      fetchCertifications(search);
    } catch (err) {
      showToast(err.response?.data?.message || "Failed to save certification mapping.", "error");
    }
  };

  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this certification mapping?")) return;
    try {
      await axiosInstance.delete(`/admin/resume-intelligence/certifications/${id}`);
      showToast("Certification mapping deleted successfully!", "success");
      fetchCertifications(search);
    } catch (err) {
      showToast("Failed to delete certification mapping.", "error");
    }
  };
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className={`mt-2 text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
            Map professional certifications to target skills they validate in the graph.
          </p>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={`${darkMode ? "bg-gray-900/60" : "bg-gray-50"}`}>
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider opacity-60">Certification Name</th>
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider opacity-60">Validated Skills</th>
                ...
                {certifications.length === 0 ? (
                  ...
                  certifications.map((item) => (
                    <tr key={item._id} className={`transition-colors ${darkMode ? "hover:bg-gray-800/40" : "hover:bg-gray-50"}`}>
                      <td className="p-4 text-sm font-semibold">{item.certificationName}</td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          ...
          >
            <h3 className="text-xl font-bold mb-4">
              {editingId ? "Edit Certification Mapping" : "Add Certification Mapping"}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto flex-1 pr-1">
              <div>
                <label className="block text-xs font-semibold mb-1 opacity-70">Certification Name</label>
                <input
                  type="text"
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                >
                  {editingId ? "Save Changes" : "Create Certification"}
                </button>
              </div>
            </form>
          </motion.div>
...
export default CertificationMapping;
