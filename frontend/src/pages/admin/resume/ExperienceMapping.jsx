// frontend/src/pages/admin/resume/ExperienceMapping.jsx
...
const ExperienceMapping = () => {
  ...
  const fetchExperiences = async (searchVal = "") => {
    setLoading(true);
    try {
      const res = await axiosInstance.get(`/admin/resume-intelligence/experience?search=${searchVal}`);
      setExperiences(res.data.experiences);
    } catch (err) {
      showToast("Failed to fetch experience mappings.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openAddModal = () => {
    setEditingId(null);
    setFormData({ experiencePattern: "", mappedSkills: [] });
  ...
  const openEditModal = (item) => {
    ...
    setFormData({
      experiencePattern: item.experiencePattern,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.experiencePattern.trim() || formData.mappedSkills.length === 0) {
      showToast("Please provide an experience phrase and select at least one skill.", "error");
      return;
    }

    try {
      if (editingId) {
        await axiosInstance.put(`/admin/resume-intelligence/experience/${editingId}`, formData);
        showToast("Experience mapping updated successfully!", "success");
      } else {
        await axiosInstance.post("/admin/resume-intelligence/experience", formData);
        showToast("Experience mapping created successfully!", "success");
      }
      setShowFormModal(false);
      fetchExperiences(search);
    } catch (err) {
      showToast(err.response?.data?.message || "Failed to save experience mapping.", "error");
    }
  };

  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this experience mapping?")) return;
    try {
      await axiosInstance.delete(`/admin/resume-intelligence/experience/${id}`);
      showToast("Experience mapping deleted successfully!", "success");
      fetchExperiences(search);
    } catch (err) {
      showToast("Failed to delete experience mapping.", "error");
    }
  };
    ...
    <div className={`min-h-screen px-6 md:px-12 py-10 transition-all duration-500 ${
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-4xl font-bold tracking-tight bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent">
            Experience Extraction Mappings
          </h1>
          <p className={`mt-2 text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
            Configure extraction mapping rules from typical employment experience phrases to target skills.
          </p>
        </div>
      ...
      <div className="mb-6 flex gap-4 max-w-md">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search experience phrases..."
            value={search}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={`${darkMode ? "bg-gray-900/60" : "bg-gray-50"}`}>
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider opacity-60">Experience Phrase / Description Pattern</th>
                  <th className="p-4 text-xs font-semibold uppercase tracking-wider opacity-60">Extracted Skills</th>
                ...
                {experiences.length === 0 ? (
                  <tr>
                    <td colSpan="3" className="p-8 text-center text-sm opacity-50">
                      No experience mapping patterns found.
                    </td>
                  </tr>
                ) : (
                  experiences.map((item) => (
                    <tr key={item._id} className={`transition-colors ${darkMode ? "hover:bg-gray-800/40" : "hover:bg-gray-50"}`}>
                      <td className="p-4 text-sm font-semibold">{item.experiencePattern}</td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          ...
          >
            <h3 className="text-xl font-bold mb-4">
              {editingId ? "Edit Experience Mapping" : "Add Experience Mapping"}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto flex-1 pr-1">
              <div>
                <label className="block text-xs font-semibold mb-1 opacity-70">Experience Pattern Phrase</label>
                <input
                  type="text"
                  placeholder="e.g. Worked with React to build frontend screens"
                  value={formData.experiencePattern}
                  onChange={(e) => setFormData({ ...formData, experiencePattern: e.target.value })}
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                >
                  {editingId ? "Save Changes" : "Create Experience Mapping"}
                </button>
              </div>
            </form>
          </motion.div>
