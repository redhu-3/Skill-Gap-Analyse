// src/pages/admin/ai/AISkillRecommender.jsx
...
const AISkillRecommender = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await axios.post(`${API}/api/ai-recommendations/generate/skills/${selectedRole}`, {}, { headers });
  ...
  return (
    <div className={`min-h-screen px-6 md:px-12 py-10 ${darkMode ? 'bg-gradient-to-br from-gray-950 via-gray-900 to-black text-white' : 'bg-gradient-to-br from-indigo-50 via-white to-violet-50 text-gray-900'}`}>
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
        ...
        <h1 className="text-4xl font-bold tracking-tight bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent">AI Skill Recommender</h1>
        <p className={`mt-2 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Discover missing skills for an existing job role.</p>
      </motion.div>
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className={base}>
          <h2 className="text-lg font-bold mb-5 flex items-center gap-2"><LightBulbIcon className="w-5 h-5 text-amber-400" /> Select Job Role</h2>
          ...
          <AIGenerateButton onClick={handleGenerate} loading={loading} label="Recommend Missing Skills" />
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          {recommendation ? (
            ...
            <div className={`${base} flex flex-col items-center justify-center h-48 text-center`}>
              <LightBulbIcon className={`w-10 h-10 mb-3 ${darkMode ? 'text-gray-700' : 'text-gray-300'}`} />
              <p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Select a role and generate skill suggestions.</p>
            </div>
          )}
        </motion.div>
      </div>
